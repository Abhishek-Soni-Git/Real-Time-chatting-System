...
const Profile = () => {
  ...
  useEffect(() => {
    if (userInfo.profileSetup) {
      setFirstNmae(userInfo.firstName);
    ...
    if (validateProfile()) {
      try {
        ...
        if(response.status === 200 && response.data) {
          ...
          //console.log("going to chat",userInfo);
  ...
  return( 
  <div className="bg-[#101010] h-[100vh] flex text-center justify-center flex-col gap-10  items-center">
    <div className="flex flex-col gap-10 w-[80vw] md:w-max">
      <div onClick={handleNavigate}>
        <IoArrowBack className="text-4xl lg:text" />
            ...
            {colors.map((color, index) => (
              <div
                className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all duration-300 
                  ${selectedColor === index ? "outlin outline-white/50 outline-1" : ""}}
                  `}
                key={index}
